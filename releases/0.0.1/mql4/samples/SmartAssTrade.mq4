...
//+------------------------------------------------------------------+
#property copyright "Copyright 2014,  Roy Philips Jacobs ~ 25/01/2014"
...
extern bool     LotsOptimization = False;
extern double               Lots = 1.0;
extern string  AutomaticSystemTP = "Set AutomaticTakeProfit=True or False";
extern bool  AutomaticTakeProfit = False;
extern string    MinimumSystemTP = "If Set AutomaticTakeProfit=False";
extern double  MinimumTakeProfit = 10;
...
double sbb,BBlt0,BBlb0,bbct0,bbct1;
...
int dev,hb,hs,ob,os,i,ip,is,it,nx,ox,z;
//---
string symbol;
string CopyR;
//----    
...
//---- 
   ...
   digit=Digits;
   if (digit==3 || digit==5)
      {pp=Point*10; dev=pp*spg; mt4ecn=true;}
...
int CalcProvit() //--function: for calculation Automatic Take Profit.
   { 
      ...
      if (!opttp) {tpAS=MinimumTakeProfit*pp; tpBS=MinimumTakeProfit*pp;}
      tp=MinimumTakeProfit*pp;
      //--
      RefreshRates();
      pBo = High[0]-Close[0];
      pSo = Close[0]-Low[0];
      ...
      if (ob<1)
         {
            ...
            if ((AccountFreeMarginCheck(symbol,OP_BUY,lot) <= 0) || (GetLastError()==134))
               {Print("-----OPEN NEW BUY ORDER ",symbol," - NOT ENOUGH MONEY..!"); RefreshRates(); return(0);}
            else  
               {      
                  ResetLastError();
                  int error;
      ...
      if (os<1)
         {
            ...
            if ((AccountFreeMarginCheck(symbol,OP_SELL,lot) <= 0) || (GetLastError()==134))
               {Print("-----OPEN NEW SELL ORDER ",symbol," - NOT ENOUGH MONEY..!"); RefreshRates(); return(0);}
            else
               {      
                  ResetLastError();
                  int error;
...
int deinit()
  {
//----
    if (!opttp) {tp=MinimumTakeProfit*pp;}
    else {tp=7*pp;}
    if (!autsl) {sl=StopLoss*pp;}
    else {sl=180*pp;}
    total=OrdersTotal();
