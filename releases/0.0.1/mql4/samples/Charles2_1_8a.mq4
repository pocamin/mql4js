...
void start()
{
   ...
   if( inited==false || (TimeCurrent() - lastBarTime)/60 >= SystemPeriod )
   {
      ...
      for(s=0; s<ArraySize(SymbolsArray); s++)
      {
         ...
         Amount = normPrice(MarketInfo(CurrentSymbol,MODE_MINLOT),CurrentSymbol);
         ...
         LotsHedgeValue = normPrice(LotsHedgeValue,CurrentSymbol);
...
int CheckForClose(int s)
{
   ...
   if(Profit<-AccountBalance()*RiskPercent/100)
   {
      ...
      int ticket=-1;
      if( blockConditionHit[s]==false && (blockedSellLots>0 || blockedBuyLots>0) && (blockedSellLots != blockedBuyLots) )
      {

         Log("[CONDIZIONE DI BLOCCAGGIO DI EMERGENZA - Balancing]: blockedBuyLots#"+blockedBuyLots+"; blockedSellLots#"+blockedSellLots);
      ...
      if( blockConditionHit[s]==true && (blockedSellLots != blockedBuyLots) && loserLots != 0 )
      {
         blockConditionHit[s]=false;
      }
      else
      {
         blockConditionHit[s]=true;
      }
   ...
   if(blockConditionHit[s]==true)
   {
      ...
      //Print(CurrentSymbol + " --> " + OpenedLoosingBuyLots+" - "+OpenedLoosingSellLots);
   
      if( OpenedLoosingBuyLots!=0 && OpenedLoosingSellLots!=0 && OpenedLoosingBuyLots == OpenedLoosingSellLots)
      {
         double balanceLots=normPrice(OpenedLoosingBuyLots,CurrentSymbol);
         if(balanceLots<normPrice(MarketInfo(CurrentSymbol,MODE_MINLOT),CurrentSymbol))
         {
            balanceLots=normPrice(MarketInfo(CurrentSymbol,MODE_MINLOT),CurrentSymbol);
         }
...
void CharlesStatus()
{
   ...
   string blocked;
   for(int bb=0;bb<ArraySize(blockConditionHit); bb++)
   {
      if(blockConditionHit[bb]==true)
      {
         blocked = "\nBlock Condition HIT";
         break;
      }
...
double LOT()
{
   ...
   int LOTDIGITS = MarketInfo(CurrentSymbol,MODE_DIGITS);

   int ecn = 1;
   if(LOTDIGITS==3||LOTDIGITS==5) ecn = 10;

   double tickvalue=1;//( (MarketInfo(CurrentSymbol,MODE_TICKVALUE)/MarketInfo(CurrentSymbol,MODE_TICKSIZE)) * (MarketInfo(CurrentSymbol,MODE_POINT)*ecn) );
   ...
   if (StopLoss>0)
   {
      riskPip = StopLoss * ecn;
   }
   else
   {
      riskPip = RiskPercent * ecn * 10;
   }
         ...
         if(((TimeCurrent()-OrderOpenTime())/60 >= PERIOD_H1) && OrderProfit()+OrderSwap()+OrderCommission()<0 && takeProfit>0)
         {
            ...
            if(UsePivots==true && isCals[index]==true)
            {
               ...
               if(chgFromSupport3<0 && MathAbs(chgFromSupport3)>=takeProfit && (OrderTakeProfit()==0 || OrderTakeProfit()>(Resist1[index]+takeProfit)))
               {
                  ...
                  {
                     Log("[Order# "+ticket+"] WARNING! Too low. Hedging and modifying stopLoss to "+normPrice(MarketInfo(OrderSymbol(), MODE_BID)-takeProfit,OrderSymbol()));
                     //OpenOrder(OrderSymbol(),NormalizeDouble(OrderLots()*xFactor,MarketInfo(OrderSymbol(),MODE_DIGITS)),OP_SELL);
                     return( OrderModify( OrderTicket(), OrderOpenPrice(), normPrice(MarketInfo(OrderSymbol(), MODE_BID)-takeProfit,OrderSymbol()), OrderTakeProfit(), 0, Blue) );
                  }
                  else
                  {*/
                     if( MathAbs(OrderTakeProfit() - (Resist1[index]+takeProfit)) > MathPow(0.1,MarketInfo(OrderSymbol(),MODE_DIGITS)) )
                     {
               ...
               {
                  if( MathAbs(OrderTakeProfit() - (Resist2[index]+takeProfit)) > MathPow(0.1,MarketInfo(OrderSymbol(),MODE_DIGITS)) )
                  {
               ...
               {
                  if( MathAbs(OrderTakeProfit() - (Resist3[index]+takeProfit)) > MathPow(0.1,MarketInfo(OrderSymbol(),MODE_DIGITS)) )
                  {
         ...
         if(((TimeCurrent()-OrderOpenTime())/60 >= PERIOD_H1) && OrderProfit()+OrderSwap()+OrderCommission()<0 && takeProfit>0)
         {
            if(UsePivots==true && isCals[index]==true)
            {
               ...
               if(chgFromResist3>0 && MathAbs(chgFromResist3)>=takeProfit && (OrderTakeProfit()==0 || OrderTakeProfit()<(Support1[index]-takeProfit)))
               {
                  ...
                  {
                     Log("[Order# "+ticket+"] WARNING! Too low. Hedging and modifying takeprofit to "+normPrice(MarketInfo(OrderSymbol(),MODE_ASK)+takeProfit, OrderSymbol()));
                     //OpenOrder(OrderSymbol(),NormalizeDouble(OrderLots()*xFactor,MarketInfo(OrderSymbol(),MODE_DIGITS)),OP_BUY);
                     return( OrderModify( OrderTicket(), OrderOpenPrice(), normPrice(MarketInfo(OrderSymbol(),MODE_ASK)+takeProfit, OrderSymbol()), OrderTakeProfit(), 0, Red) );
                  }
                  else
                  {*/
                     if( MathAbs(OrderTakeProfit() - (Support1[index]-takeProfit)) > MathPow(0.1,MarketInfo(OrderSymbol(),MODE_DIGITS)) )
                     {
               ...
               {
                  if( MathAbs(OrderTakeProfit() - (Support2[index]-takeProfit)) > MathPow(0.1,MarketInfo(OrderSymbol(),MODE_DIGITS)) )
                  {
               ...
               {
                  if( MathAbs(OrderTakeProfit() - (Support3[index]-takeProfit)) > MathPow(0.1,MarketInfo(OrderSymbol(),MODE_DIGITS)) )
                  {
...
//---------------------------------------------------------------------------         
//---------------------------------------------------------------------------
double normPrice (double price, string symboll)
{
   if(MarketInfo(CurrentSymbol, MODE_LOTSTEP) == 0.01){return(NormalizeDouble(price,2));}
   else if(MarketInfo(CurrentSymbol, MODE_LOTSTEP) == 0.1){return(NormalizeDouble(price,1));}
   else {
         double otherStep = MathMod(price,MarketInfo(CurrentSymbol, MODE_LOTSTEP));
         price -= otherStep;
   }
}
//---------------------------------------------------------------------------
