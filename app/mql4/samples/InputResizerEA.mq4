...
#import "user32.dll"
  ...
  int InsertMenuA(int hMenu, int uPosition, int uFlags, int uIDNewItem, string lpNewItem);
  ...
  bool InvalidateRect(int hWnd, int rect[4], bool bErase);
#import
...
extern bool   WeekendMode        = false;

//+--------------------------------------------------------------------------------------+
void init(){if (!IsDllsAllowed()) Alert("InputResizerEA: DLLs not allowed!"); if (WeekendMode) start();}
//+--------------------------------------------------------------------------------------+
void deinit(){}
//+--------------------------------------------------------------------------------------+
void start()
   ...
   while (true) // We start an infinite loop
      ...
      if (pWnd == fWnd) // so if we caught a window...
         ...
         if (!resizable) // it's not resizable until we do it:
            ...
            SetWindowLongA(pWnd, GWL_STYLE, GetWindowLongA(pWnd, GWL_STYLE) | WS_SIZEBOX | WS_MINIMIZEBOX | WS_MAXIMIZEBOX);
            // The above alone doesn't do, the window has to have the items in right-click-on
            // titlebar menu:
            int menu = GetSystemMenu(pWnd, false);
            InsertMenuA(menu, 1, MF_BYPOSITION | MF_STRING, SC_SIZE, "Size");
            InsertMenuA(menu, 1, MF_BYPOSITION | MF_STRING, SC_MAXIMIZE, "Maximize");
            InsertMenuA(menu, 1, MF_BYPOSITION | MF_STRING, SC_MINIMIZE, "Minimize");
            InsertMenuA(menu, 1, MF_BYPOSITION | MF_STRING, SC_RESTORE, "Restore");
            ...
            if (RememberSize) 
               ...
               if (Individual) // one for all ar one for one?!
                  ...
                  gvAdd = "                                                                                       ";
                  GetWindowTextA(pWnd, gvAdd, 50); // retrieve EA/indicator name
      ...
      if (Period() == PERIOD_MN1) break;
      // 300-500 ms does not eat resources but still warrants prompt reaction:
      Sleep(SleepTime);
      }   
   }
