...
extern string 	StartHour = "08:00";
extern string 	CloseHour = "00:00";
...
if (TimeCurrent()-StrToTime(StartHour)>=0 && (TimeCurrent()-StrToTime(StartHour))<=600) {

for (int shift=1; shift<=limit; shift++) {

if ( High[shift]<High[shift+1] && High[shift+1]<High[shift+2] && Low[shift]>Low[shift+1] && Low[shift+1]>Low[shift+2]) {
	Conflict=true;
}
else {
	Conflict=false;
	...
	if (Low[shift]>Low[shift+1] && Low[shift+1]>Low[shift+2] && Conflict==false) {	
		if (Reverse) SellValue=1; else BuyValue=1;		
		break;
	}

	if (High[shift]<High[shift+1] && High[shift+1]<High[shift+2] && Conflict==false) {
...
if (TimeCurrent()-StrToTime(CloseHour)>=0 && (TimeCurrent()-StrToTime(CloseHour))<=900) {
	CloseBuy=1;
	CloseSell=1;
