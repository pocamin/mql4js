...
double Trend = iMA(NULL,0,MAP,0,MODE_EMA,PRICE_CLOSE,1);


//close on SL

  if (Close[1]<Trend && OrdersTotal()>0 && maReversal == true)
  { 
    CloseAllBuy();
    TradeType = 2;
  ...
  if (OrdersTotal()==(MaxTrade-1) && allowSL==true)
  {
  ...
  if (OrdersTotal()==3 && (MaxTrade-1)>=OrdersTotal())
  {
  if(OrderSelect((OrdersTotal()-1), SELECT_BY_POS)==true)
  {
   if ((OrderType()==OP_BUY) && ((OrderOpenPrice()-OrderClosePrice())/Point)>= TP)
   {
          ticket=OrderSend(Symbol(),OP_BUY,Lots*L3,Ask,3,0,0,"MLTrendETF",16384,0,Green);
         if(ticket>0)
           {
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) sendEmail();
           }
   ...
   if ((OrderType()==OP_SELL) && ((OrderClosePrice()- OrderOpenPrice())/Point)>= TP)
   {
            ticket=OrderSend(Symbol(),OP_SELL,Lots*L3,Bid,3,0,0,"MLTrendETF",16384,0,Green);
         if(ticket>0)
           {
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) sendEmail();
           }
  ...
  if (OrdersTotal()==4 && (MaxTrade-1)>=OrdersTotal())
  {
  if(OrderSelect((OrdersTotal()-1), SELECT_BY_POS)==true)
  {
   if ((OrderType()==OP_BUY) && ((OrderOpenPrice()-OrderClosePrice())/Point)>= TP)
   {
          ticket=OrderSend(Symbol(),OP_BUY,Lots*L4,Ask,3,0,0,"MLTrendETF",16384,0,Green);
         if(ticket>0)
           {
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) sendEmail();
           }
   ...
   if ((OrderType()==OP_SELL) && ((OrderClosePrice()- OrderOpenPrice())/Point)>= TP)
   {
            ticket=OrderSend(Symbol(),OP_SELL,Lots*L4,Bid,3,0,0,"MLTrendETF",16384,0,Green);
         if(ticket>0)
           {
            if(OrderSelect(ticket,SELECT_BY_TICKET,MODE_TRADES)) sendEmail();
           }
