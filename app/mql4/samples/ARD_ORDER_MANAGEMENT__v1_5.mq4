//+------------------------------------------------------------------+
//|                                    ARD order managemetn v1.5.mq4 |
//|                           Copyright © 2009, ARDIANSYAH WOHAN ARD |
//|                                        http://www.WohaDirect.com |
//+------------------------------------------------------------------+
#property copyright "Copyright © 2009, ARDIANSYAH WOHAN ARD"
#property link      "http://www.WohaDirect.com"
#define BUY  1 
#define SELL 2
#define CLOSE 3
#define MODIFY 4
...
extern double sl = 50,tp = 100,lotsize = 5,m_sl=20,m_tp=100,pp=50,lotmax=0.1;
...
int start()
  {
   if(iDeMarker(NULL,0,2,1)<0.5&&iDeMarker(NULL,0,2,2)>0.5)//use this for buy signal
   {
    if(status==1) return(0);
    else orders(BUY);
   }
   if(iDeMarker(NULL,0,2,1)>0.5&&iDeMarker(NULL,0,2,2)<0.5)//use this for sell signal
   {
                     ...
                     if(Symbol() == OrderSymbol())
                        {
                           ...
                           if(cls==false){ 
                              ...
                              if (err==135) { RefreshRates();i++;}
                                 ...
                                 {
                                    Print("Modify Order Failed == with error ",err); 
                                    return(false);  
                                    break;
                                   }
                               }
