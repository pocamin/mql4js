//+------------------------------------------------------------------+
//|                     Unseasonalised JPY Average Trading Range.mq4 |
//|                                                    Gary Thompson |
...
int init()
  {
   ...
   Array_Pointer = Sample_Counter-1;
      ...
      ATR = (Sum_Total * 100) / (Sample_Counter-1);      
...
int start()
  {
      ...
      {
      Difference = ATR - (Array_of_Ranges[Array_Pointer] * 100);
      ...
      ATR_Above_Open = NormalizeDouble((Open[0] + (ATR/100)), Digits);
      ATR_Below_Open = NormalizeDouble((Open[0] - (ATR/100)), Digits); 
      ...
      {
      Sum_of_XY += (Sample_Counter - Trend_Analysis_Counter)*(Array_of_Ranges[Trend_Analysis_Counter] * 100);
      ...
      {
      Trend_Analysis_Counter--;
      if(((Array_of_Ranges[Trend_Analysis_Counter] * 100) - (M *(Sample_Counter - Trend_Analysis_Counter) + C)) < 0)
         {
         Sum_of_APE +=((((Array_of_Ranges[Trend_Analysis_Counter] * 100) - (M *(Sample_Counter - Trend_Analysis_Counter) + C)) * (-1))
                       /(Array_of_Ranges[Trend_Analysis_Counter] * 100)) * 100;
         }
      else if(((Array_of_Ranges[Trend_Analysis_Counter] * 100) - (M *(Sample_Counter - Trend_Analysis_Counter) + C)) > 0)
         {
         Sum_of_APE +=(((Array_of_Ranges[Trend_Analysis_Counter] * 100) - (M *(Sample_Counter - Trend_Analysis_Counter) + C))
                       /(Array_of_Ranges[Trend_Analysis_Counter] * 100)) * 100;
         }         
      }
      ...
      LTF_Above_Open = NormalizeDouble((Open[0] + (Trend_Forecasted_Range/100)), Digits);
      LTF_Below_Open = NormalizeDouble((Open[0] - (Trend_Forecasted_Range/100)), Digits);
      LTF_MAPE_Above_Open = NormalizeDouble((Open[0] + ((Trend_Forecasted_Range*(1 - (Trend_Forecasted_MAPE /100)))/100)), Digits);
      LTF_MAPE_Below_Open = NormalizeDouble((Open[0] - ((Trend_Forecasted_Range*(1 - (Trend_Forecasted_MAPE /100)))/100)), Digits);
