...
extern double             StopLoss = 108; // SL adjusted by user, default 88
//---
//--- Global scope
double pp,sl,tp;
double ask,bid,dev;
...
//----
      ...
      bep=NormalizeDouble(pp*6.2,Digits);
               ...
               if(OrderSymbol()==_Symbol)
                 {
                   if(OrderType()==OP_BUY)
                     {
                       //---
                       if((stxb==0)&&(OrderStopLoss()==0.00||OrderTakeProfit()==0.00))
                         {
                   ...
                   if(OrderType()==OP_SELL)
                     {
                       //---
                       if((stxs==0)&&(OrderStopLoss()==0.00||OrderTakeProfit()==0.00))
                         {
...
//----
     ...
     CkClose();     
     CkOpen();
     if((ob+os>0)&&((stxb+stxs==0)||opttp||TrlStp))
       {TpSlTrlS();}
     //----
     return;
//---- 
