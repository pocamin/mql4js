...
extern int    RangeFilter                     = 0;      // (amount pips) minimum body of candle for trading signal
extern double StopLoss                        = 30;     // (amount pips) stop-loss; 0 = without stop-loss
extern double TakeProfit                      = 0;      // (amount pips) take profit; 0 = without take profit
extern double MoveToBE                        = 15;     // (amount pips) profit when move the stop-loss level to opening price; 0 = without move to BE (option is disabled)
...
extern int    TrailingLimit                   = 5;      // (amount pips) distance to pending order
extern int    TrailingDistance                = 5;      // (amount pips) trailing stop; 0 = without trailing-stop
extern int    MAPeriod                        = 3;
extern int    MAShift                         = 0;
extern int    MAMethod                        = MODE_LWMA; // (0-3)
extern double MaxSpreadWithCommission         = 15.0;   // (amount pips)
...
extern bool   UseFilterKeltnerChannel         = true;   // true = use indicator "Keltner Channels"; false = does not use indicator "Keltner Channels"
extern int    KC_Period                       = 10;     // parameter for indicator "Keltner Channels"
...
int start()
  {
   ...
   double ma    = NormalizeDouble(iMA(NULL,0,MAPeriod,MAShift,MAMethod,PRICE_CLOSE,0),digits);
   ...
   if(range > rangeFilter)
     { 
      if(close < ma && bar_direction < 0)
        {
         if(!UseFilterKeltnerChannel || close < kc_lower) direction++; // SELL
        }
      else
        {
         if(close > ma && bar_direction > 0)
           {
            if(!UseFilterKeltnerChannel || close > kc_upper) direction--; // BUY
           }
        }
     }
   direction /= is_ok;
   ...
   for(i=OrdersTotal()-1; i>=0; i--)
     {
      ...
      if(order_type == OP_BUYLIMIT || order_type == OP_SELLLIMIT) continue;
      num_orders++;
   ...
   if(ShowComment || WriteDebugLog)
     {
      ...
      string message = StringConcatenate("Copyrighted by www.sbginter.com & www.eaihub.com\n",
