...
int start()
{
  ...
  {
     int result=0;
     if((iMA(NULL,0,2,0,MODE_EMA,PRICE_CLOSE,2)<iMA(NULL,0,35,0,MODE_EMA,PRICE_CLOSE,2))&&(iMA(NULL,0,2,0,MODE_EMA,PRICE_CLOSE,1)>iMA(NULL,0,35,0,MODE_EMA,PRICE_CLOSE,1))) // Here is your open buy rule
     {
     ...
     if((iMA(NULL,0,2,0,MODE_EMA,PRICE_CLOSE,2)>iMA(NULL,0,35,0,MODE_EMA,PRICE_CLOSE,2))&&(iMA(NULL,0,2,0,MODE_EMA,PRICE_CLOSE,1)<iMA(NULL,0,35,0,MODE_EMA,PRICE_CLOSE,1))) // Here is your open Sell rule
     {
  ...
  for(int cnt=0;cnt<OrdersTotal();cnt++)
     {
      ...
      if(OrderType()<=OP_SELL &&   
         ...
         )  
        {
         if(OrderType()==OP_BUY)  
           {
              if((iMA(NULL,0,2,0,MODE_EMA,PRICE_CLOSE,2)>iMA(NULL,0,35,0,MODE_EMA,PRICE_CLOSE,2))&&(iMA(NULL,0,2,0,MODE_EMA,PRICE_CLOSE,1)<iMA(NULL,0,35,0,MODE_EMA,PRICE_CLOSE,1))) //here is your close buy rule
              {
           ...
           {
                if((iMA(NULL,0,2,0,MODE_EMA,PRICE_CLOSE,2)<iMA(NULL,0,35,0,MODE_EMA,PRICE_CLOSE,2))&&(iMA(NULL,0,2,0,MODE_EMA,PRICE_CLOSE,1)>iMA(NULL,0,35,0,MODE_EMA,PRICE_CLOSE,1))) // here is your close sell rule
                {
                   OrderClose(OrderTicket(),OrderLots(),OrderClosePrice(),Slippage,Red);
                }
            if(TrailingStop>0)  
