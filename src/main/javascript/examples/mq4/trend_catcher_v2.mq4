...
extern bool Reverse_Sig_Open=false;
// if true, then a signal will be inverted, instead of buy will be opened sell, instead of sell - buy will be opened
   ...
   for(int h_1=accTotal1-1;h_1>=0;h_1--)
      if(OrderSelect(h_1,SELECT_BY_POS,MODE_HISTORY)==true)
         if(OrderSymbol()==Symbol())
            if(OrderMagicNumber()==Magic)
              {
               if(OrderType()==OP_BUY || OrderType()==OP_SELL) {h_Percent=OrderProfit()+OrderSwap()+OrderCommission(); Lot_History=OrderLots();break;}
              }
   for(int h_2=accTotal1-1;h_2>=0;h_2--)
      if(OrderSelect(h_2,SELECT_BY_POS,MODE_HISTORY)==true)
         if(OrderSymbol()==Symbol())
            if(OrderMagicNumber()==Magic)
              {
               if(OrderOpenTime()>=iTime(Symbol(),Candle_TF,0)) {h++;}
               if(OrderOpenTime()<iTime(Symbol(),Candle_TF,0)) {break;}
...
//------Day for trading----
   ...
   if(DayOfWeek()==1&&Monday==true)   {trade_day=true;}
   if(DayOfWeek()==2&&Tuesday==true)  {trade_day=true;}
   if(DayOfWeek()==3&&Wednesday==true){trade_day=true;}
   if(DayOfWeek()==4&&Thursday==true) {trade_day=true;}
   if(DayOfWeek()==5&&Friday==true)   {trade_day=true;}
...
//_________lot count by the percentage of the account balance and apply the Martingale according to the settings________
...
//-----------open orders-----
   if(Reverse_Sig_Open==false&&trade_day==true&&n==0&&sig==1&&h==0&&sl>min_sl) {int open = OrderSend(Symbol(),OP_BUY,Lot,Ask,Slip,Ask-sl,Ask+tp,NULL,Magic,0,0);Print("Цена buy=",Ask,", Стоп лосс=",Ask-sl,", Тейк профит=",Ask+tp);}
   if(Reverse_Sig_Open==false&&trade_day==true&&n==0&&sig==2&&h==0&&sl>min_sl) {int open = OrderSend(Symbol(),OP_SELL,Lot,Bid,Slip,Bid+sl,Bid-tp,NULL,Magic,0,0);Print("Цена sell=",Bid,", Стоп лосс=",Bid+sl,", Тейк профит=",Bid-tp);}

   if(Reverse_Sig_Open==true&&trade_day==true&&n==0&&sig==2&&h==0&&sl>min_sl) {int open = OrderSend(Symbol(),OP_BUY,Lot,Ask,Slip,Ask-sl,Ask+tp,NULL,Magic,0,0);Print("Цена buy=",Ask,", Стоп лосс=",Ask-sl,", Тейк профит=",Ask+tp);}
   if(Reverse_Sig_Open==true&&trade_day==true&&n==0&&sig==1&&h==0&&sl>min_sl) {int open = OrderSend(Symbol(),OP_SELL,Lot,Bid,Slip,Bid+sl,Bid-tp,NULL,Magic,0,0);Print("Цена sell=",Bid,", Стоп лосс=",Bid+sl,", Тейк профит=",Bid-tp);}
            ...
            if(OrderMagicNumber()==Magic)
              {
               if(OrderType()==OP_BUY)
                 {
                  if(sig==2&&Close_opposite_signal==true&&Reverse_Sig_Open==false) {int cl = OrderClose(OrderTicket(),OrderLots(),Bid,Slip,0);}
                  if(sig==1&&Close_opposite_signal==true&&Reverse_Sig_Open==true)  {int cl = OrderClose(OrderTicket(),OrderLots(),Bid,Slip,0);}
                 }
               if(OrderType()==OP_SELL)
                 {
                  if(sig==1&&Close_opposite_signal==true&&Reverse_Sig_Open==false) {int cl = OrderClose(OrderTicket(),OrderLots(),Ask,Slip,0);}
                  if(sig==2&&Close_opposite_signal==true&&Reverse_Sig_Open==true)  {int cl = OrderClose(OrderTicket(),OrderLots(),Ask,Slip,0);}
                 }
              }
  }
//+------------------------------------------------------------------+
